...
export class ColorsService {
    ...
    interpolateHelper( x, valMin, valMax, colorArrayMin, colorArrayMax ): Number[] {
        ...
        for ( let i = 0; i < 4; i++ ) {
            let val = ( x - valMin ) / ( valMax - valMin );
            if ( val > 1 ) {
                val = ( 1 - ( val % 1 ) );
            }
            colorArray.push( Math.abs( ( ( colorArrayMax[ i ] - colorArrayMin[ i ] ) * val ) + colorArrayMin[ i ] ) );
        }
     ...
     */
    setSelectedColors( color1d: string, color3d: string, colorMgitm: string ) {
        this.stored1dColor = color1d;
        this.stored3dColor = color3d;
        this.storedMgitmColor = colorMgitm;
